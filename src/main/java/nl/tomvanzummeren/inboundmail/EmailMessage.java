...
/**
 * Represents a <code>MimeMessage</code> as an email message. This makes it easy to:
 *
 * <ul>
...
public class EmailMessage {
     ...
     *
     * @param mimeMessage email mime message
     * @throws MessagingException when an error occurs reading the mime message
     * @throws IOException        when an error occurs working with I/O
     */
     ...
     * @return the body of the requested type or <code>null</code> if it doesn't exist
     */
    ...
    /**
     * Initializes the fields of this email by parsing the mime message this class was constructed with.
     *
     * @throws MessagingException when an error occurs reading the mime message
     * @throws IOException        when an error occurs working with I/O
     */
    private void parseMimeMessage() throws MessagingException, IOException {
        ...
        toAddresses = (InternetAddress[]) mimeMessage.getRecipients(Message.RecipientType.TO);
        ccAddresses = (InternetAddress[]) mimeMessage.getRecipients(Message.RecipientType.CC);
        bccAddresses = (InternetAddress[]) mimeMessage.getRecipients(Message.RecipientType.BCC);
    ...
    /**
     * Parses the content of the mime message recursively. The given content can be of type <code>InputStream</code>,
     * <code>String</code> or <code>Multipart</code>. If the content is of the latter type it contains multiple parts.
     * Each part can by itself by either of the three types mentioned before.
     *
     * <p/>This method keeps running recursively until no <code>Multipart</code> content objects are left.
     *
     * @param content     content to parse
     * @param contentType type of the content (e.g. "text/plain")
     * @throws MessagingException when an error occurs reading the mime message
     * @throws IOException        when an error occurs working with I/O
     */
    private void parseContent(Object content, String contentType) throws IOException, MessagingException {
        if (content instanceof Multipart) {
            ...
            attachments.add(new Attachment(contentType.toLowerCase(), inputStream));
